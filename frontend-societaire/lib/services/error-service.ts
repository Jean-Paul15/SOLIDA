...
export class ApiError extends Error {
  ...
  constructor(kind: ApiErrorKind, code: string, status: number, message: string, details?: unknown) {
    super(message);
    this.kind = kind;
    this.code = code;
    this.status = status;
...
const DEFAULT_MESSAGE: Record<Exclude<ApiErrorKind, "domain">, string> = {
  ...
  network: "Connexion impossible pour le moment. Votre demande est conservée sur ce téléphone.",
};

/**
 * Même forme que le pendant agent (frontend/lib/services/error-service.ts) : deux
 * types de payload backend, {code,message} métier ou {detail} FastAPI natif — mais
 * pas de notion d'accès refusé (aucun rôle côté sociétaire) ni de redirection vers un
 * écran de connexion (il n'y en a pas ici, cf. section 2 du parcours).
 */
export async function throwIfError(response: Response): Promise<void> {
  ...
  if (Array.isArray(body?.detail)) {
    throw new ApiError("validation", "validation_failed", response.status, DEFAULT_MESSAGE.validation, body.detail);
  }
