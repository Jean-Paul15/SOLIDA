import { describe, expect, it } from "vitest";
import { calculateMonthlyInstallment, calculateDebtRatio } from "./credit";

describe("calculateMonthlyInstallment", () => {
  it("applique la formule d'annuité constante à taux mensuel positif", () => {
    // Vérifié indépendamment : i=0.015, n=12 -> facteur (1.015)^12 ≈ 1.195618
    // echeance = 1 200 000 * 0.015 * 1.195618 / (1.195618 - 1) ≈ 110 016
    expect(calculateMonthlyInstallment(1200000, 12, 0.015)).toBe(110016);
  });

  it("se réduit à une répartition linéaire à taux nul", () => {
    expect(calculateMonthlyInstallment(1200000, 12, 0)).toBe(100000);
  });

  it("renvoie zéro si la durée est nulle ou négative", () => {
    expect(calculateMonthlyInstallment(1200000, 0, 0.015)).toBe(0);
    expect(calculateMonthlyInstallment(1200000, -3, 0.015)).toBe(0);
  });
});

describe("calculateDebtRatio", () => {
  it("additionne charges et échéance sur le revenu", () => {
    expect(calculateDebtRatio(40000, 200000, 60000)).toBeCloseTo(0.5);
  });

  it("renvoie zéro si le revenu est nul ou négatif", () => {
    expect(calculateDebtRatio(40000, 0, 60000)).toBe(0);
  });
});
