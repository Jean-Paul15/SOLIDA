...
import { bornesZones, positionSurJauge, segmentsJauge, SCORE_MAX, SCORE_MIN } from "./jauge-score";

describe("positionSurJauge", () => {
  it("place le score minimal à 0%", () => {
    expect(positionSurJauge(SCORE_MIN)).toBe(0);
  });

  it("place le score maximal à 100%", () => {
    expect(positionSurJauge(SCORE_MAX)).toBe(100);
  });

  it("borne un score hors plage", () => {
    expect(positionSurJauge(SCORE_MIN - 100)).toBe(0);
    expect(positionSurJauge(SCORE_MAX + 100)).toBe(100);
  });

  it("place un score médian proportionnellement", () => {
    const milieu = (SCORE_MIN + SCORE_MAX) / 2;
    expect(positionSurJauge(milieu)).toBeCloseTo(50, 5);
  });
});

describe("bornesZones et segmentsJauge", () => {
  const configuration = {
    version_grille: "v1",
    grille: {
      marge: 0.1,
  ...
  it("produit 4 segments couvrant toute la jauge dans l'ordre croissant", () => {
    const segments = segmentsJauge(bornesZones(configuration));
